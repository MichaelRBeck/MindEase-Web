...
type CognitiveContextValue = {
    ...
    applyPatch: (patch: Partial<CognitivePreferences>) => Promise<void>; // ✅ novo
    resetDraft: () => void;
...
function needsToCognitiveOverrides(profile: UserProfile): Partial<CognitivePreferences> {
    const { needs } = profile;
...
export function CognitiveProvider({ children }: { children: React.ReactNode }) {
    ...
    // ✅ estado (para UI/efeitos) + ref (para evitar race)
